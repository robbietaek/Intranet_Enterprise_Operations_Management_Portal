...
public interface VacationMapper {
 ...
 + " values (#{num},#{id},#{dept},#{startdate},#{enddate},#{vacationtype},#{state},#{grp})")
	int insert(Vacation vacation);

	@Select({"<script>",
		   "select count(*) from vacation where state = ${state} ",
	...
	List<Vacation> select(Map<String, Object> map);
			...
			@Select({"<script>",
			    "select * from vacation where state = 2 ",
			    " <if test='id != null'> and id = #{id}</if>",
			    " <choose>",
			    " <when test='num != null'> and num = #{num} </when>",
			    " <otherwise>"
			    ...
			    + "</otherwise>",
			     "</choose>",
			    "</script>"})
		List<Vacation> adminselect(Map<String, Object> map);
	...
	@Update("update  vacation  set state= 1 where num=#{num}")
	int update(int num);
